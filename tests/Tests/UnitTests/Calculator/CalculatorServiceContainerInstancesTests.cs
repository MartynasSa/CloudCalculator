...
public class CalculatorServiceContainerInstancesTests(WebApplicationFactory<Program> factory) : TestBase(factory)
{
    ...
    [Fact]
    public async Task CalculateCostComparisonsAsync_WithContainerInstances_SmallUsage_ReturnsValidResult()
    {
        // Arrange
        ...
        var calculationRequest = new CalculationRequest
        {
            ...
            {
                Computes = [ComputeType.ContainerInstances]
            }
        };
        ...
        Assert.Contains(ComputeType.ContainerInstances, result.Resources.Computes);
        Assert.Equal(3, result.CloudCosts.Count);
        
        foreach (var cloudCost in result.CloudCosts)
        {
            Assert.Contains(cloudCost.CostDetails, cd => cd.ResourceSubCategory == ResourceSubCategory.ContainerInstances);
        }
    ...
    [Fact]
    public async Task CalculateCostComparisonsAsync_WithContainerInstances_MediumUsage_ReturnsValidResult()
    {
        // Arrange
        var service = GetService();
        var calculationRequest = new CalculationRequest
    ...
    [Fact]
    public async Task CalculateCostComparisonsAsync_WithContainerInstances_LargeUsage_ReturnsValidResult()
    {
        // Arrange
        var service = GetService();
        var calculationRequest = new CalculationRequest
    ...
    [Fact]
    public async Task CalculateCostComparisonsAsync_WithContainerInstances_ExtraLargeUsage_ReturnsValidResult()
    {
        // Arrange
        var service = GetService();
        var calculationRequest = new CalculationRequest
    ...
    [Fact]
    public async Task CalculateCostComparisonsAsync_WithContainerInstances_CostsIncreaseWithUsageSize()
    {
        // Arrange
        var service = GetService();
        ...
        Assert.True(largeAwsCost.TotalMonthlyPrice >= smallAwsCost.TotalMonthlyPrice,
            "Larger usage size should have equal or higher costs for ContainerInstances");
